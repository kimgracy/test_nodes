...
2. Change the path of local yolov5
   ...
   modify '/test_ws/src/yolo_detection/yolo_detection/yolo_detector.py'
...
# Build
my_bboxes_msg 관련 dependency 문제는 해결해 놨으니, 그냥 아래에 적힌 대로 colcon build 한번만 실행해도 된다. (해당 repo에 아직 auto landing 관련 코드는 없기 때문에 auto landing은 dependency 잘 고려해서 build 문제 없도록 코드 올리기!)
```
cd test_ws
...
2. 수정 사항을 본인 repo에 올리기
   ```
   git add .
   git commit -m "[YOUR_COMMIT_MESSAGE]" # e.g. ADD: yolo_test_00
   git push origin main
   ```

3. 본인 repo에 들어가서 commit이 제대로 됐는지 클릭해서 확인하고 Compare & Pull reqeust 버튼을 클릭한다. 본인이 어떤 코드를 작성했는지 **자세하게** 설명하고 reviewer에 okj001010을 추가 후, Create pull request 버튼을 클릭한다.

4. 여기까지 한 이후에는 코드 확인 후 직접 merge 할 예정이니, (아마 안 되긴 할텐데) **절대** 바로 merge 버튼을 누르지 말아야 한다.


# Run

```
ros2 run test_nodes test_00
ros2 run test_nodes test_01
ros2 run test_nodes test_02 --ros-args --params-file ~/test_ws/src/test_nodes/config/test_02_waypoint.yaml
```
- test_00: arming
- test_01: takeoff → land
- test_02: takeoff → N 2m → NE 2m → E 2m → home → land (사각 비행)
