// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from my_bboxes_msg:msg/VehiclePhase.idl
// generated code does not contain a copyright notice
#include "my_bboxes_msg/msg/detail/vehicle_phase__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `phase`
#include "rosidl_runtime_c/string_functions.h"

bool
my_bboxes_msg__msg__VehiclePhase__init(my_bboxes_msg__msg__VehiclePhase * msg)
{
  if (!msg) {
    return false;
  }
  // phase
  if (!rosidl_runtime_c__String__init(&msg->phase)) {
    my_bboxes_msg__msg__VehiclePhase__fini(msg);
    return false;
  }
  return true;
}

void
my_bboxes_msg__msg__VehiclePhase__fini(my_bboxes_msg__msg__VehiclePhase * msg)
{
  if (!msg) {
    return;
  }
  // phase
  rosidl_runtime_c__String__fini(&msg->phase);
}

bool
my_bboxes_msg__msg__VehiclePhase__are_equal(const my_bboxes_msg__msg__VehiclePhase * lhs, const my_bboxes_msg__msg__VehiclePhase * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // phase
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->phase), &(rhs->phase)))
  {
    return false;
  }
  return true;
}

bool
my_bboxes_msg__msg__VehiclePhase__copy(
  const my_bboxes_msg__msg__VehiclePhase * input,
  my_bboxes_msg__msg__VehiclePhase * output)
{
  if (!input || !output) {
    return false;
  }
  // phase
  if (!rosidl_runtime_c__String__copy(
      &(input->phase), &(output->phase)))
  {
    return false;
  }
  return true;
}

my_bboxes_msg__msg__VehiclePhase *
my_bboxes_msg__msg__VehiclePhase__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_bboxes_msg__msg__VehiclePhase * msg = (my_bboxes_msg__msg__VehiclePhase *)allocator.allocate(sizeof(my_bboxes_msg__msg__VehiclePhase), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(my_bboxes_msg__msg__VehiclePhase));
  bool success = my_bboxes_msg__msg__VehiclePhase__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
my_bboxes_msg__msg__VehiclePhase__destroy(my_bboxes_msg__msg__VehiclePhase * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    my_bboxes_msg__msg__VehiclePhase__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
my_bboxes_msg__msg__VehiclePhase__Sequence__init(my_bboxes_msg__msg__VehiclePhase__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_bboxes_msg__msg__VehiclePhase * data = NULL;

  if (size) {
    data = (my_bboxes_msg__msg__VehiclePhase *)allocator.zero_allocate(size, sizeof(my_bboxes_msg__msg__VehiclePhase), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = my_bboxes_msg__msg__VehiclePhase__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        my_bboxes_msg__msg__VehiclePhase__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
my_bboxes_msg__msg__VehiclePhase__Sequence__fini(my_bboxes_msg__msg__VehiclePhase__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      my_bboxes_msg__msg__VehiclePhase__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

my_bboxes_msg__msg__VehiclePhase__Sequence *
my_bboxes_msg__msg__VehiclePhase__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  my_bboxes_msg__msg__VehiclePhase__Sequence * array = (my_bboxes_msg__msg__VehiclePhase__Sequence *)allocator.allocate(sizeof(my_bboxes_msg__msg__VehiclePhase__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = my_bboxes_msg__msg__VehiclePhase__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
my_bboxes_msg__msg__VehiclePhase__Sequence__destroy(my_bboxes_msg__msg__VehiclePhase__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    my_bboxes_msg__msg__VehiclePhase__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
my_bboxes_msg__msg__VehiclePhase__Sequence__are_equal(const my_bboxes_msg__msg__VehiclePhase__Sequence * lhs, const my_bboxes_msg__msg__VehiclePhase__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!my_bboxes_msg__msg__VehiclePhase__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
my_bboxes_msg__msg__VehiclePhase__Sequence__copy(
  const my_bboxes_msg__msg__VehiclePhase__Sequence * input,
  my_bboxes_msg__msg__VehiclePhase__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(my_bboxes_msg__msg__VehiclePhase);
    my_bboxes_msg__msg__VehiclePhase * data =
      (my_bboxes_msg__msg__VehiclePhase *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!my_bboxes_msg__msg__VehiclePhase__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          my_bboxes_msg__msg__VehiclePhase__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!my_bboxes_msg__msg__VehiclePhase__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
