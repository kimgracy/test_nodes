...
class VehicleController(Node):
    ...
    def __init__(self):
        ...
        self.back_transition_ratio = 0.500                  # WP6 to WP7 internal divide ratio
    ...
    def main_timer_callback(self):
        ...
        if self.phase == -1:
            ...
            if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_AUTO_LOITER:
                ...
                self.previous_goal_pos = self.pos
                self.current_goal_pos = self.WP[1]
                self.bezier_counter = 0
                self.bezier_points = self.bezier_curve(self.previous_goal_pos, self.current_goal_pos)
                self.phase = 1

        elif self.phase == 1:
            if self.subphase == "MC":
                if self.bezier_counter == self.num_bezier - int(1 / self.time_period) - 1:
                    self.publish_trajectory_setpoint(position_sp=self.current_goal_pos)
                else:
                ...
                if np.linalg.norm(self.pos - self.current_goal_pos) < self.mc_acceptance_radius:
                    ...
                    self.mission_yaw = self.get_bearing_to_next_waypoint(self.previous_goal_pos, self.current_goal_pos)
