    ...
    def main_timer_callback(self):
        ...
        if self.phase == -1:
            if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
                ...
                # vehicle command send repeatedly until the vehicle is armed
                self.phase = 0
            ...
            if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_AUTO_LOITER:
                ...
                self.print("Change to mission mode completed")
                self.print("Start Mission!\n")
                self.phase = 1
        
        elif self.phase == 1:
            if self.vtol_vehicle_status.vehicle_vtol_state == VtolVehicleStatus.VEHICLE_VTOL_STATE_MC   \
                and np.linalg.norm(self.pos - self.WP[7]) < self.mc_acceptance_radius:
                if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_AUTO_MISSION:
                    ...
                    self.publish_vehicle_command(
                        VehicleCommand.VEHICLE_CMD_DO_SET_MODE, 
                        param1=1.0, # main mode
                        param2=4.0  # offboard mode
                    )
                elif self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_AUTO_OFFBOARD:
                    self.print("Offboard control mode completed")
                    self.print("Start moving to home position\n")
