    ...
    def main_timer_callback(self):
        if self.phase == -1:
            if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
                ...
                self.print("Takeoff requested. Takeoff by publishing setpoint.\n")
            ...
            self.publish_trajectory_setpoint(position_sp = self.pos + (self.current_goal - self.pos) / np.linalg.norm(self.current_goal - self.pos) * self.mc_speed) # slow mc flight
            if np.linalg.norm(self.pos - self.current_goal) < self.mc_acceptance_radius:
                self.print("Takeoff completed. Go to phase 1.")
                self.print("Stay in same position for 8 seconds.\n")
                self.phase = 1
            ...
            if self.time_checker >= 8.0:
                self.time_checker = 0
                self.print("Start Landing. Go to phase 2.\n")
                self.phase = 2
